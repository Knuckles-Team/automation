  ...
  if ! command -v ntfs-3g &> /dev/null; then
    ...
    if [[ ${config_flag} == "true" ]]; then
      ...
      if grep -q '^/dev/sdc2' /etc/fstab; then
        ...
        echo -e "/dev/sdc2 ${file_storage} ntfs-3g rw,auto,user,permissions,uid=1000,gid=1000,umask=0000,noatime,nodiratime,nofail,nodev,nosuid,exec 0 0" | sudo tee -a /etc/fstab
      fi
      if grep -q '^/dev/sdd2' /etc/fstab; then
        sudo sed -i "s#/dev/sdd2.*#/dev/sdd2 ${photos} ntfs-3g rw,auto,user,permissions,uid=1000,gid=1000,umask=0000,noatime,nodiratime,nofail,nodev,nosuid,exec 0 0#" /etc/fstab
      else
